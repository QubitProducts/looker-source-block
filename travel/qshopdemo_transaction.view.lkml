view: qshopdemo_transaction {
  ...
  dimension: weeks_since_first_entry {
    ...
    description: "Number of weeks between the first view of a visitor and a certain transaction. QP fields: meta_ts, meta_recordDate"
  }
  
  dimension: days_since_first_entry {
    type: number
    sql: DATEDIFF(MSEC_TO_TIMESTAMP(${TABLE}.meta_ts), TIMESTAMP((DATE(${{TRACKING_ID}_qp_bi_view_v01.visitor_first_entry_date} )))) ;;
    label: "Days Since First Entry"
    group_label: "Time Since First Entry"
    value_format_name: decimal_0
    description: "Number of days between the first view of a visitor and a certain transaction. QP fields: meta_ts, meta_recordDate"
  }
  ...
  dimension_group: time_data_points {
    ...
    description: "Timestamp of the transaction. QP fields: meta_ts"
  }
  ...
  dimension_group: time_data_points_previous_transaction {
    ...
    description: "Timestamp of the previous transaction of the visitor. QP fields: meta_ts"
  }
  ...
  measure: converters {
    ...
    description: "Count of unique visitor_ids on page views that are labeled with any non-null transaction_id. If above 1.000.000, the result is approximated. QP fields: context_id, transaction_id"
  }

  measure: transactions {
    type: number
    sql:  EXACT_COUNT_DISTINCT(${transaction_id}) ;;
    description: "Count of unique transaction_ids (always exact count). QP fields: transaction_id"
  }

  measure: average_order_value {
    type: number
    sql:  SUM(${TABLE}.package_total_baseValue) / EXACT_COUNT_DISTINCT(${transaction_id}) ;;
  ...
  measure: revenue_per_converter {
    type: number
    sql:  SUM(${TABLE}.transaction_total) / COUNT(DISTINCT ${TABLE}.context_id, 1000000) ;;
    value_format_name: decimal_2
  ...
  measure: session_conversion_rate {
    type: number
    sql: COUNT(DISTINCT ${TABLE}.session_id, 1000000) /  COUNT(DISTINCT ${{TRACKING_ID}_qp_bi_view_v01.session_id}, 1000000) ;;
  ...
  measure: visitor_conversion_rate {
    type: number
    sql: COUNT(DISTINCT ${TABLE}.context_id, 1000000) /  COUNT(DISTINCT ${{TRACKING_ID}_qp_bi_view_v01.context_id}, 1000000) ;;
    value_format_name: percent_2
  ...
  measure: revenue_per_visitor {
    type: number
    sql: ${{TRACKING_ID}_qp_bi_transaction_v01.sum_of_transaction_total} / ${{TRACKING_ID}_qp_bi_view_v01.view_visitors} ;;
    value_format_name: decimal_2
